...
const HealthCheck = () => {
        ...
        const checkApiHealth = async () => {
            try {
                ...
                if (data.status === "ok" && data.message) {
                    ...
                    setHealthStatus({ status: "misconfigured", message: "Unexpected API response format." });
                }
            } catch (err) {
                console.error("API health check failed:", err);
                setHealthStatus({ status: "disconnected", message: "API is not reachable." });
            }
        };
    ...
    const fetchGroupHealthStatus = async (group, setGroupHealthStatus) => {
        try {
            ...
            setGroupHealthStatus({ status: "error", message: `Failed to fetch ${group} health status` });
        }
    };
    ...
    const renderHealthStatus = (status, group) => {
        ...
        return (
            <div className={`alert ${alertClass} mt-2`}>
                <p><strong>{group} Status:</strong> {status.status}</p>
                <p><strong>{group} Message:</strong> {status.message}</p>
            </div>
        );
    };
    ...
    return (
        <div className="container my-5">
            <header className="text-center mb-4">
                ...
                <p className="lead">Monitor the health status of the API and user groups.</p>
            </header>
            <main>
                <section className="mb-4">
                    <h2>API Health</h2>
                    {healthStatus ? (
                        <div className={`alert ${healthStatus.status === "connected" ? "alert-success" : "alert-danger"}`}>
                ...
                <section className="mb-4">
                    ...
                    <div className="mb-3">
                        <button
                            ...
                            onClick={() => fetchGroupHealthStatus("admin", setAdminHealthStatus)}
                        >
                            Check Admin Health
                        </button>
                    ...
                    <div className="mb-3">
                        <button
                            ...
                            onClick={() => fetchGroupHealthStatus("user", setUserHealthStatus)}
                        >
                            Check User Health
                        </button>
                    ...
                    <div className="mb-3">
                        <button
                            ...
                            onClick={() => fetchGroupHealthStatus("employee", setEmployeeHealthStatus)}
                        >
