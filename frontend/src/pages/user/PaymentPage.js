...
const PaymentPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchReservation = async () => {
      ...
      try {
        ...
        const response = await api.get(`/reservation/get-details/${reservationId}`, {
  ...
  const handlePayment = async () => {
    try {
      ...
      await api.post(
          ...
          {
            headers: {Authorization: `Bearer ${token}`},
            params: {reservation_id: reservationId, method: selectedMethod, region},
          }
      );
  ...
  const { reservation, movie_details, seat_details, hall_name, show_start_time } =
    reservationData || {};
  ...
  <div className="container mt-4">
    ...
    {reservationData && (
      <div className="card p-4 mb-4 shadow-sm">
        ...
        <p><strong>Movie:</strong> {movie_details?.title}</p>
        <p><strong>Runtime:</strong> {movie_details?.runtime} min</p>
        <p><strong>Hall:</strong> {hall_name}</p>
        <p><strong>Show Time:</strong> {new Date(show_start_time).toLocaleString()}</p>
        <p><strong>Seats:</strong> {seat_details?.map(
    ...
    <div className="mb-4">
      ...
      <div className="row">
        {[
          ...
          { id: "apple_pay", label: "Apple Pay", image: "/assets/applepay.png" },
          { id: "paypal", label: "PayPal", image: "/assets/paypal.png" }
        ].map((method) => (
          <div className="col-md-4 mb-3" key={method.id}>
            <div
              className={`card payment-option ${selectedMethod === method.id ? "border-success" : ""}`}
